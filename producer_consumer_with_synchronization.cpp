...
pthread_cond_t check_empty= PTHREAD_COND_INITIALIZER;
pthread_cond_t check_full= PTHREAD_COND_INITIALIZER;
...
void* takeOrders(void* arg)
{
	...
	for(int i = 0; i < MAX; ++i) {
		...
		if(num_new_orders==N){
            pthread_cond_wait(&check_full, &data_mutex);
		}
        // Get user input
	    ...
	    pthread_cond_signal(&check_empty);
...
void* processOrders(void* arg)
{
	...
	for(int i = 0; i < MAX; ++i) {
		// Beginning of critical region 3
			pthread_mutex_lock(&data_mutex);
            if(num_new_orders==0){
            	pthread_cond_wait(&check_empty, &data_mutex);
		}
        // Get u
		...
		pthread_cond_signal(&check_full);
